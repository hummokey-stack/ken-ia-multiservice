...
Une fois l'installation terminée, votre Jarvis personnel est opérationnel. Vous pouvez commencer à lui parler, lui demander de l'aide sur vos projets, lui faireprendre des notes, et bien plus.
...
├── .env                         # Secrets et clés API (ne pas partager)
...
├── deliverables/
│   ├── web/                     # Code, sites web, composants
│   ├── content/                 # Posts, scripts vidéo, textes produits
│   ├── strategy/                # Plans, roadmaps, analyses
│   └── automation/              # Scripts, workflows, configs API
├── .claude/
│   ├── commands/
...
- Copiez `.env.example` en `.env` et remplissez vos clés réelles
- Le fichier `.env` est dans `.gitignore` et ne doit jamais être partagé ni commité
- Consultez `.env.example` pour la liste complète des services pris en charge

---

## Livrables

Tout ce que Jarvis produit est rangé dans `deliverables/` selon le type :
- `web/` pour le code et les projets web
- `content/` pour les textes, posts et scripts vidéo
- `strategy/` pour les analyses, plans et roadmaps
- `automation/` pour les scripts et workflows

Convention de nommage : `AAAA-MM-JJ_nom-du-livrable.md`
